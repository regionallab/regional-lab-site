...
function SaiAbout() {
  const members = [
    { role: '代表 / Founder & CEO', jp: '山田 一郎', en: 'Ichiro Yamada', bio: '南魚沼に出会い、地方創生に身を投じる。学生時代より地域課題と向き合い、Regional Labを創業。' },
    { role: '副代表 / Co-founder & COO', jp: '佐藤 智樹', en: 'Tomoki Sato', bio: '事業戦略・財務を統括。組織の土台を築き、複数事業を並行で成長へと導く。' },
    { role: 'Re.Event 事業統括責任者', jp: '鈴木 美咲', en: 'Misaki Suzuki', bio: 'イベント企画・地域連携の専門家。秋保・蔵王の地域密着型ビジコンを率いる。' },
    { role: 'Re.Lab 事業統括責任者', jp: '田中 蓮', en: 'Ren Tanaka', bio: 'コーチング・人材育成を担当。一人一人の理想に伴走し、コミュニティの質を担保する。' },
  ];
      ...
      <div style={{ padding: '96px 56px', display: 'grid', gridTemplateColumns: '280px 1fr', gap: 56 }}>
        <div>
          ...
          <div style={{ marginTop: 24 }}>
            <Placeholder label="founder / 南魚沼の景" ratio="4/5" color1="#d0bf95" color2="#bfa97a" textColor={saiB.ink} />
          </div>
        </div>
        <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 32 }}>
          {[
            ...
            <div key={label} style={{ borderLeft: `2px solid ${saiB.akane}`, paddingLeft: 20 }}>
              ...
              <div style={{ fontFamily: saiBFont.jpSerif, fontSize: 18, lineHeight: 1.9, marginTop: 10, letterSpacing: '0.04em' }}>{body}</div>
            </div>
          ))}
        </div>
      </div>
          ...
          {members.map((m, i) => (
            <div key={m.role} style={{ background: saiB.paperDeep, padding: 28, display: 'grid', gridTemplateColumns: '180px 1fr', gap: 24 }}>
              ...
              <div>
                ...
                <p style={{ fontSize: 13, lineHeight: 2, color: saiB.ink, margin: 0 }}>{m.bio}</p>
              </div>
            </div>
          ))}
        </div>
  ...
  const partners = [
    {
      ...
      name: '株式会社 顧問名鑑',
      en: 'Komon Meikan, Inc.',
      relation: '経営顧問・ネットワーク提供',
      body: '顧問人材データベースを擁する企業様。Regional Lab の経営・事業設計において、各分野のトップ顧問の知見を提供いただいています。',
      tags: ['顧問', '経営支援', 'ネットワーク'],
      color: saiB.ai,
    },
    {
      ...
      name: '株式会社 遊然',
      en: 'Yuzen, Inc.',
      relation: 'マーケティング商材・コンテンツ提供',
      body: 'Re.Event「Event Lab」におけるマーケティング学習コンテンツを提供いただいているパートナー様。集客の本質を学べる教材をご一緒に。',
      tags: ['マーケティング', 'コンテンツ', 'Event Lab連携'],
      color: saiB.akane,
    },
    {
      no: '03',
      ...
      body: '地方創生を共に推進する団体様。地域での活動・知見の共有を通じ、互いに地方の未来を彩る取り組みを行なっています。',
      tags: ['地方創生', 'コミュニティ', '共同企画'],
      color: saiB.koke,
    },
  ];
